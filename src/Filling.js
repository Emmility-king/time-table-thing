...
function Filling() {
  ...
  const opt1 = [];
  const opt2 = [];
  for (let i = 0; i <= 50; i++) opt.push(<option> {i} </option>);
  for (let i = 0; i <= 6; i++) opt2.push(<option> {i} </option>);
  ...
  for (let i = 0; i <= no_of_days; i++) opt1.push(<option> {i} </option>);
  ...
  function getAllHours() {
    // console.log('1');
    if (no_of_faculties === 0) return true;
    // console.log('2');
    if (no_of_classes === 0) return true;
    // console.log('3');
    ...
    for (let i = 1; i <= no_of_classes; i++) {
      for (let j = 1; j <= no_of_faculties; j++) {
        ...
        week_hours_all[j - 1] =
          parseInt(week_hours_all[j - 1]) + parseInt(week_hours[i - 1][j - 1]);
    ...
    for (let j = 1; j <= no_of_faculties; j++) {
      if (
        week_hours_all[j - 1] >
        no_of_Lectures * (no_of_days - off_days_fac[j - 1])
      ) {
        alert(
          "Lectures limit exceeded for faculty " +
            j +
            " (" +
            faculty_names[j - 1] +
            ") by " +
  ...
  function AlgorithmA() {
    for (let i = 1; i <= no_of_classes; i++) {
      ...
      let p = 0,
        ...
        loop = 0;
      for (let j = 1; j <= no_of_faculties; j++) {
        if (week_hours[i - 1][j - 1] === 0) continue;
        ...
        while (wh !== 0) {
          ...
          if (
            time_table_checker[j - 1][Math.floor(p / no_of_days)][
              Math.floor(p % no_of_days)
            ]
          ) {
            p++;
          ...
          if (
            time_table[i - 1][Math.floor(p / no_of_days)][
              Math.floor(p % no_of_days)
            ] === 0
          ) {
            wh--;
    ...
    <div>
      ...
      <Form onSubmit={fill_Faculties()}>
        ...
        <Details
          ...
          heading={
            <div>
              <h2>Faculties Data</h2>
              <mark>
                <strong>
                  <i>
                    <sub>(Tip - Just enter short names for reference...)</sub>
                  </i>
                </strong>
              </mark>
            </div>
      ...
      <Form onSubmit={fill_classes()}>
        ...
        <Details
          ...
          heading={
            <div>
              <h2>
                Class Lectures Data <br />
              </h2>
              <sub>
                <strong>
                  <i>
                    Enter number of hours/week for every faculty.
                    <br />
                    <mark>(Default value will be 0)</mark>
                  </i>
                </strong>
              </sub>
            </div>
      ...
      <div className="w-full h-full absolute z-10 flex justify-center items-center backdrop-lg">
        <Tablize
          show={algo}
          data={col_data}
          heading={
            <div>
              <h2>
                Time-Tables for Students
                <br />
              </h2>
              <mark>
                <sub>
                  <strong>
                    <i>(Remember To Print Out Table Before Leaving!)</i>
                  </strong>
                </sub>
              </mark>
            </div>
